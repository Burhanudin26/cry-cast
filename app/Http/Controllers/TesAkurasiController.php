...
class TesAkurasiController extends Controller
{
        ...
        public function HitungSMA($table)
        {
            ...
            // Prepare the SQL query to get the low, high, and volume values from the tesakurasi table in groups of 5
        ...
        public function Threshold($table)
        {
            ...
            // Prepare the SQL query to get the monthly averages of low, high, and volume from the tesakurasi table
        ...
        // treshold check based on threshold table per month and iterate to chech per day in tesakurasi table
        public function bayes($table)
        {
            ...
            foreach ($thresholds as $threshold) {
                ...
                // Get the tesakurasi data for the month
        ...
        public function predict()
        {
            ...
            // count all data in table tesakurasi and iterate
        ...
        public function import(Request $request)
        {
            ...
            if ($file && $file->isValid()) {
                ...
                $table = 'tesakurasi';
                DB::table('tesakurasi')->where('id', '<>', 'admin')->delete();
                foreach ($data as $row) {
            ...
            // get data as array from table tesakurasi and column low and column id
            $low_data = DB::table($table)->select('low')->get();
            $low_trend = DB::table('SMA')->select('sma_low')->get();
    
            // get data as array from table tesakurasi and column volume and column id
            ...
            return view('outputacc')->with(compact('data', 'trend', 'low_data', 'low_trend', 'volume_data', 'volume_trend', 'date', 'output', 'outputb', 'akurasi', 'datei'));
        }
    
        public function index(Request $request)
        {
            // minim date in table tesakurasi
            $minDate = DB::table('tesakurasi')->min('date');
            $maxDate = DB::table('tesakurasi')->max('date');
            return view('menu.tesakurasi', compact('minDate', 'maxDate'));
        }
